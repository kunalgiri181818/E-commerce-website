...
if(isset($_POST['add']))
{
	$sql_insert = "INSERT INTO `og_product_packets` SET `product_id` = '".addslashes($_POST['hid_product_id'])."',`packet_size` = '".addslashes($_POST['packet_size'])."',`discount` = '".addslashes($_POST['discount'])."',`original_price` = '".addslashes($_POST['original_price'])."',`price` = '".addslashes($_POST['price'])."'";
	$exe_insert = mysql_query($sql_insert) or die(mysql_error());
	
	$_SESSION['add_succ_msg'] = 'Packet added successfully.';
	header("location: packet_list2.php?product_id=".$_POST['hid_product_id']);
	exit;
}
...
if(isset($_POST['edit']))
{
	$sql_update = "UPDATE `og_product_packets` SET `product_id` = '".addslashes($_POST['hid_product_id'])."',`packet_size` = '".addslashes($_POST['packet_size'])."',`discount` = '".addslashes($_POST['discount'])."',`original_price` = '".addslashes($_POST['original_price'])."',`price` = '".addslashes($_POST['price'])."' WHERE `packet_id` = '".$_REQUEST['packet_id']."'";
...
if(isset($_REQUEST['action']) && $_REQUEST['action'] == "edit") 
{
	...
	$price = stripslashes($fetch_record['price']);
}
?>
	...
	<div class="workplace">
		<div class="row-fluid">
			<div class="span12">
				...
				<input type="hidden" name="packet_id" id="packet_id" value="<?php echo $fetch_record['packet_id'];?>" />
				...
				<div class="block-fluid">
					...
					<div class="row-form clearfix">
						<div class="span3">Discount <font color="#FF0000">*</font></div>
						<div class="span9"><input type="text" name="discount" id="discount" value="<?php echo $discount;?>" style="width:50%;" />%</div>
					</div>				
					<div class="row-form clearfix">
					...
					<div class="row-form clearfix">
						...
						<?php } ?>
							<button class="btn" type="button" name="back" onclick="window.location.href='packet_list2.php?product_id=<?php echo $_GET['product_id']?>'">Back To Packet List For <?php echo $fetch_product['product_name_english'];?></button>
						</div>
					</div>
				</div>
				</form>
