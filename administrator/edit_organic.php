...
if(isset($_POST['edit']))
{
	...
	$sql_update = "UPDATE `og_contents` SET `content_title` = '".addslashes($_POST['content_title'])."',`content` = '".addslashes($_POST['content'])."',`content_photo` = '".$content_photo."' WHERE `content_id` = '9'";
	$exe_update = mysql_query($sql_update) or die(mysql_error());
	
	$_SESSION['save_succ_msg'] = 'Content updated successfully.';
	header("location: edit_organic.php");
	exit;
}

//FETCH DATA FROM DATABASE
$fetch_record = mysql_fetch_array(mysql_query("SELECT * FROM `og_contents` WHERE `content_id` = '9'"));
		...
		<div class="row-fluid">
			<div class="span12">
				<div class="head clearfix">
					...
					<h1>Organic Benefits</h1>
				</div>
				<form action="" method="post" enctype="multipart/form-data" onsubmit="return frm_validation();">
				<input type="hidden" name="hid_content_photo" id="hid_content_photo" value="<?php echo $content_photo;?>" />
				<div class="block-fluid">
					...
					<div class="row-form clearfix">
						...
						<br /><img src="../uploads/banner/<?php echo $content_photo;?>" border="0" alt="" />
						</div>                            
					 </div>
					<div class="row-form clearfix">
						<div class="span3">&nbsp;</div>
