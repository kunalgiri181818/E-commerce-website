...
if(isset($_POST['edit']))
{
	$sql_update = "UPDATE `og_contents` SET `content_title` = '".addslashes($_POST['content_title'])."',`content` = '".addslashes($_POST['content'])."' WHERE `content_id` = '2'";
	$exe_update = mysql_query($sql_update) or die(mysql_error());
	
	$_SESSION['save_succ_msg'] = 'Content updated successfully.';
	header("location: edit_terms.php");
	exit;
}

//FETCH DATA FROM DATABASE
$fetch_record = mysql_fetch_array(mysql_query("SELECT * FROM `og_contents` WHERE `content_id` = '2'"));
