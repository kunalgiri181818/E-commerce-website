...
$user_counter = 0;
$total_anniversary_user = 0;
$sql_record = "SELECT * FROM `og_users` WHERE `anniversary_date` <> '0000-00-00' ORDER BY `user_id` DESC";
...
if($num_record>0)
{
	...
	{
		$explode = explode("-",$fetch_record['anniversary_date']);	
		...
		{
			$user_counter = 1;
			$total_anniversary_user = $total_anniversary_user+$user_counter;
		}
		else
		{
			$user_counter = 0;
